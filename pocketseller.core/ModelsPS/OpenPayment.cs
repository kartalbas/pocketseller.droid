...
namespace pocketseller.core.Models
{
    public class OpenPayment : BaseModel
	{
        ...
        public static ObservableCollection<OpenPayment> Find(string addressNr)
        {
            var objResult = DataService.PocketsellerConnection.Table<OpenPayment>()
                .Where(a => (a.Adressnumber == addressNr))
                .OrderBy(a => a.Docdate).ToList();
        ...
        public static bool IsWithinPaymentDays(string addressNr, int days)
        {
            var opList = Find(addressNr);
