...
#region Alert Dialogs
        ...
        public override IDisposable PasswordChange(PasswordChangeConfig config)
        {
            var activity = this.TopActivityFunc();
            if (activity is AppCompatActivity act)
                return this.ShowDialog<PasswordChangeDialogFragment, PasswordChangeConfig>(act, config);

            return this.Show(activity, () => new PasswordChangeBuilder().Build(activity, config));
        }
