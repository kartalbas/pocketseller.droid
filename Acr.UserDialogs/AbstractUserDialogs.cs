...
namespace Acr.UserDialogs
{
    ...
    public abstract class AbstractUserDialogs : IUserDialogs
    {
        ...
        public abstract IDisposable PasswordChange(PasswordChangeConfig config);
        ...
        public virtual Task<PasswordChangeResult> PasswordChangeAsync(string title, string message, CancellationToken? cancelToken = null)
        {
            return this.PasswordChangeAsync(new PasswordChangeConfig
            {
                Title = title ?? PasswordChangeConfig.DefaultTitle,
                Message = message
            }, cancelToken);
        }


        public virtual async Task<PasswordChangeResult> PasswordChangeAsync(PasswordChangeConfig config, CancellationToken? cancelToken = null)
        {
            if (config.OnAction != null)
                throw new ArgumentException(NO_ONACTION);
            ...
            var disp = this.PasswordChange(config);
            using (cancelToken?.Register(() => Cancel(disp, tcs)))
            {
                return await tcs.Task;
            }
