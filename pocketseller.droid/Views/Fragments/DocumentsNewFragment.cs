...
namespace pocketseller.droid.Views.Fragments
{
    public sealed class DocumentsNewFragment : MvxFragment
    {
        ...
        private void SendOrder(Document document, ESettingType targetType)
        {
            ...
            var newDocument = DocumentsNewViewModel.DataService.Find<Document>(documentId);
            newDocument.Documentdetails = document.Documentdetails;

            if (!OpenPayment.IsWithinPaymentDays(document.AdressNr, 60))
            {
